    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                logger.error(f"پاسخ خام: {response.text[:500]}")
                
                # تلاش برای استخراج متن از پاسخ خام
                raw_text = response.text
                if "content" in raw_text:
                    # تلاش برای استخراج محتوا
                    try:
                        # حذف کاراکترهای اضافی
                        clean_text = raw_text.split('"content": "')[1].split('"')[0]
                        await processing.delete()
                        await update.message.reply_text(clean_text)
                    except:
                        await processing.edit_text("⚠️ خطا در پردازش پاسخ")
                else:
                    await processing.edit_text("⚠️ خطا: پاسخ نامعتبر")
                    
        elif response.status_code == 429:
            await processing.edit_text("⚠️ درخواست زیاد! لطفاً کمی صبر کنید و دوباره امتحان کنید.")
            
        elif response.status_code == 401:
            await processing.edit_text("⚠️ خطا: کلید API نامعتبر است!")
            
        elif response.status_code == 402:
            await processing.edit_text("⚠️ خطا: اعتبار تمام شده!")
        ...
        await processing.edit_text("❌ خطا: اتصال به سرور برقرار نشد!")
